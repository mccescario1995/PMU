    ...
    def fit(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.warning(f"ARIMA (SARIMAX) fit failed with order {self.order}, seasonal_order {self.seasonal_order}, falling back to (1,0,1): {e}")
            self.result = self._SARIMAX(train, order=(1, 0, 1), seasonal_order=(0, 0, 0, 0)).fit(**fit_kwargs)
